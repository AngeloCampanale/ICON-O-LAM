...
!-------------------------------------------------------------------------
  ...
  USE mo_grid_config,        ONLY: l_ocean_limited_area
...
!!$    &                 vtk_int                             , &
  ...
  !----------------------------------------------------------------------------
  ! limited area mode parameters
  INTEGER :: ocean_latbc_itype = 0 ! 0 - constant BCs, 1 - time dependent BCs --- not implemented yet and ignored
  REAL(wp) :: ocean_latbc_dtime = -1.0_wp
  CHARACTER(LEN=filename_max) :: ocean_latbc_filepattern = &
    &                            "BC_oce_R<nroot>B<jlev>_<y><m><d><h>.nc"
  CHARACTER(LEN=max_char_length) :: ocean_latbc_path = "./latbc"
  LOGICAL :: ocean_latbc_from_vn = .false.

  NAMELIST/ocean_limarea_nml/ ocean_latbc_itype, &
    & ocean_latbc_dtime, &
