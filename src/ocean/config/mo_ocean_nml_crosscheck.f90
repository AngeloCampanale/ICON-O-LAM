...
!<Optimize:inUse>
  SUBROUTINE ocean_crosscheck()
    ...
    IF (l_ocean_limited_area) THEN
#ifdef _OPENACC
      CALL finish(method_name,"Ocean limited area does not yet support ACC computations")
#endif
      IF (ocean_latbc_itype > 0 .AND. ocean_latbc_dtime < 0) &
        CALL finish(method_name,"Ocean limited area requires time difference between latbc snapshots")

      IF (ocean_latbc_dtime > 86400._wp) &
        CALL finish(method_name, "Namelist setting of dtime_latbc (ocean_limarea_nml) too large for mtime conversion!")

      CALL warning(method_name, "Code is not checking but ocean_latbc_dtime must be a multiple of model timestep!")
    ENDIF
