...
MODULE mo_hydro_ocean_run
  ...
  USE mo_grid_config,            ONLY: n_dom, l_ocean_limited_area
  ...
  USE mo_ocean_math_operators,   ONLY: update_height_depdendent_variables, check_cfl_horizontal, check_cfl_vertical, &
    & rot_vertex_ocean_3d, map_edges2vert_3d
  ...
  USE mo_ocean_time_events,      ONLY: ocean_time_nextStep, isCheckpoint, isEndOfThisRun, &
    & newNullDatetime, get_OceanCurrentTime
  ...
  USE mo_ocean_limarea,          ONLY: update_ocean_latbc, apply_ocean_ssh_latbc, &
...
!<Optimize:inUse>
  SUBROUTINE perform_ho_stepping( patch_3d, ocean_state, p_ext_data,    &
    ...
    TYPE(datetime), POINTER             :: previous_time    => NULL()
    LOGICAL :: lzacc
...
#include "adify_oes_hydro_ocean_run_perform_ho_stepping_declarations.inc"
    ...
    CALL deallocateDatetime(previous_time)
...
!         sea_ice,0)
    ...
    SUBROUTINE ocean_time_step_zstar()
        ...
        jstep = jstep + 1
        ! update model date and time mtime based
        previous_time = get_OceanCurrentTime()
        current_time = ocean_time_nextStep()
        ...
        IF (l_ocean_limited_area) CALL update_ocean_latbc(patch_3d, ocean_state, current_time, &
                                                        & previous_time, operators_coefficients, jstep)
...
!          & patch_3D%p_patch_1d(1)%prism_thick_flat_sfc_c(:,:,:), sea_ice, p_oce_sfc)
        ...
        ! Make sure this is the correct place to update the boundary
        IF (l_ocean_limited_area) &
          & CALL apply_ocean_ssh_latbc(patch_3d, ocean_state, nnew(jg)) ! Update the SSH on the boundary
        ...
        IF (l_ocean_limited_area) &
          & CALL apply_ocean_velocity_latbc(patch_3d, ocean_state, nnew(jg)) ! Update the velocity on the boundary
...
!          & patch_3D%p_patch_1d(1)%prism_thick_flat_sfc_c(:,:,:), sea_ice, p_oce_sfc)
        ...
        IF (no_tracer>=1) THEN
          CALL nudge_ocean_tracers( patch_3d, ocean_state(jg), lacc=lzacc)
          IF (l_ocean_limited_area) &
            & CALL apply_ocean_tracer_latbc(patch_3d, ocean_state, nnew(jg)) ! Update the tracers on the boudnary
        ENDIF
...
!<Optimize:inUse>
  SUBROUTINE write_initial_ocean_timestep(patch_3d,ocean_state,p_oce_sfc,sea_ice, &
    ...
    ELSE IF ( ( vert_cor_type == 1 ) ) THEN
      ocean_state%p_prog(nnew(1))%eta_c = ocean_state%p_prog(nold(1))%eta_c
      ! There should be a namelist switch to tell if stretch_c was loaded
    ...
    ! Calculate initial vorticity (this tells us how much the eddies were distroyed)
    CALL map_edges2vert_3d(patch_2d, ocean_state%p_prog(nold(1))%vn, &
      & operators_coefficients%edge2vert_coeff_cc, ocean_state%p_diag%p_vn_dual)
    CALL rot_vertex_ocean_3d(patch_3d, ocean_state%p_prog(nold(1))%vn, &
      & ocean_state%p_diag%p_vn_dual, operators_coefficients, ocean_state%p_diag%vort)
    ! sync not needed here, but used for example for the Leith (comment copied from elsewhere with surrounding lines)
    CALL sync_patch_array(sync_v, patch_2d, ocean_state%p_diag%vort)
