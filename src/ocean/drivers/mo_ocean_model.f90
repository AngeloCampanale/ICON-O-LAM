...
MODULE mo_ocean_model
  ...
  ! Horizontal grid
  !
  USE mo_grid_config,         ONLY: n_dom, use_dummy_cell_closure, l_ocean_limited_area
...
!<Optimize:inUse>
  SUBROUTINE destruct_ocean_model()
    ...
    ! Ocean LAM cleanup
    IF (l_ocean_limited_area) CALL destruct_ocean_latbc()
...
!    IF (.not. isRestart()) &
    ...
    ! Call LAM initialization (precalculate weights, load 2 time levels)
    IF (l_ocean_limited_area) CALL init_ocean_latbc(ocean_patch_3d, operators_coefficients)
