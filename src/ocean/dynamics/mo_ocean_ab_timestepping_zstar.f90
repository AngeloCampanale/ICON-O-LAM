...
!ICON_OMP_PARALLEL_DO PRIVATE(blockNo,start_edge_index,end_edge_index, je, jk, z_grad_h_block) ICON_OMP_DEFAULT_SCHEDULE
    DO blockNo = edges_in_domain%start_block, edges_in_domain%end_block
      ...
      ! FIXME: How to properly apply OpenMP parallelization
      IF (.NOT. l_ocean_limited_area) THEN ! If not LAM do normal stuff, else don't update the boundary
        DO je = start_edge_index, end_edge_index
          ...
          END DO
        END DO
      ELSE ! IF (l_ocean_limited_area)
        DO je = start_edge_index, end_edge_index
          IF (patch%edges%refin_ctrl(je,blockNo) == 0 .OR. &
            & patch%edges%refin_ctrl(je,blockNo) > 8) THEN
            DO jk = 1, patch_3d%p_patch_1d(1)%dolic_e(je,blockNo)
               vn_new(je,jk,blockNo) = (vn_pred(je,jk,blockNo) &
                 & - gdt_x_ab_beta * z_grad_h_block(je))
            END DO
        ...
        DO jc = start_cell_index, end_cell_index
          IF (patch_3d%p_patch_1d(1)%dolic_c(jc,blockNo) > 0) THEN
            IF (patch_2D%cells%refin_ctrl(jc,blockNo) == 0 .OR. &
              & patch_2D%cells%refin_ctrl(jc,blockNo) > 4) THEN
              ocean_state%p_aux%p_rhs_sfc_eq(jc,blockNo) = ( ( eta(jc,blockNo) &
                & - dtime * div_z_depth_int_c(jc)) * inv_gdt2)
            ELSE
