...
!ICON_OMP_END_PARALLEL_DO
  ...
  SUBROUTINE tracer_diffusion_vertical_implicit_zstar_onBlock( &
    ...
    DO cell_index = start_index, end_index
      ...
      IF (l_ocean_limited_area .AND. & ! Skip boundary interpolation zone in LAM
        & patch_2D%cells%refin_ctrl(cell_index,blockNo) > 0 .AND. &
        & patch_2D%cells%refin_ctrl(cell_index,blockNo) <= 4) CYCLE
  ...
  SUBROUTINE advect_individual_tracers_zstar( patch_3d, transport_state, &
    ...
    DO jb = cells_in_domain%start_block, cells_in_domain%end_block
      ...
      DO jc = start_cell_index, end_cell_index
        !FIXME: Should we let him calculate the div_adv_flux_vert and then CYCLE?
        IF (l_ocean_limited_area .AND. & ! Skip boundary interpolation zone in LAM
          & patch_2D%cells%refin_ctrl(jc,jb) > 0 .AND. &
          & patch_2D%cells%refin_ctrl(jc,jb) <= 4) CYCLE
